...
page_fault_handler(struct Trapframe *tf)
{
	...
	if ((tf->tf_cs & 0x1) == 0) {
		// Print trap with error then panic
		print_trapframe(tf);
		panic("page_fault_handler: page fault found in kernel. Faulting addr: %08x", fault_va);
	}
	...
	if (curenv->env_pgfault_upcall) {
		// Create user trap frame (UTrapframe)
